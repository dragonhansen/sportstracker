...
class Scraper:
    ...
    def scrape_pcs(self):
        ...
        if response.status_code == 200:
            ...
            if self.state["cycling_state"] == soup:
                print("No change in data, arboting scrape!")
                return
            self.state["cycling_state"] = soup
    ...
    def scrape_gprs(self):
        ...
        if response.status_code == 200:
            ...
            # Bail out if the website has not changed since last scrape
            if self.state["f1_state"] == soup:
                print("No change in data, arboting scrape!")
                return
            self.state["f1_state"] = soup
            ...
            for row in rows:
                ...
                upcoming_race = ({"Date": self.__convertMonthToNumber(date_text), "Race": gp_text})
